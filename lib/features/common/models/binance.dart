...
class Binance with _$Binance {
  const factory Binance(
      ...
      final double lastId,
      final double count) = _Binance;

  factory Binance.fromJson(final Map<String, dynamic> json) =>
      _$BinanceFromJson(json);

  static List<Binance> dummyGen() => List.filled(120, null)
      .map((final _) => const Binance('Dummy', '', '0.00', '', '', '', '',
          '100', '', '', '', '', '', '', '', '', 0, 0, 0, 0, 0))
      .toList();

  static List<Binance> fromJsonToList(final List<dynamic> list) =>
      Either.tryCatch(
          // ignore: unnecessary_lambdas
          () => list.map((final curr) => Binance.fromJson(curr)).toList(),
          (final e, final _) => e).getOrElse((final _) => []);
}
